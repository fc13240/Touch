...
function _repalce_content(dir){
	...
	_replace(path.join(dir, 'j/embed-final.gts'), function(content){
		return content.replace(/\.\/js\/blur\.js/g, './j/blur.gts');
	});
	_replace(path.join(dir, 'j/echarts.gts'), function(content){
		return content.replace(/\.js/g, '.gts');
	});
	_replace(path.join(dir, 'j/services.gts'), function(content){
